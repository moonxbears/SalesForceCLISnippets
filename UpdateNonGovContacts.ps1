...
$queriedMerl = "SELECT Id, OwnerId FROM Contact WHERE MailingState in ($($merlDel)) AND OwnerId != '$($merlId)' AND GovernmentLU__c = false"
$queriedShane = "SELECT Id, OwnerId FROM Contact WHERE MailingState in ($($shaneDel)) AND OwnerId != '$($shaneId)' AND GovernmentLU__c = false"
...
if ($setToMerlArr.Count -gt 1) 
{
    ...
    sf data update bulk --file "merlfile.csv" --sobject Contact --async -w 30
}
if ($setToShaneArr.Count -gt 1) 
{
    $setToShaneArr | Export-Csv -Path "shanefile.csv"
    sf data update bulk --file "shanefile.csv" --sobject Contact --async -w 30
}
